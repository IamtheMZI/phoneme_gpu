...
__global__ void nn_diff_add(double* output, double* output_add, int column_size){
	   int i = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void nn_weight_update(double* input, double* weight, double learning_rate, int location, int column_size){
	   int i = blockDim.x * blockIdx.x + threadIdx.x;
...
int find_minimum(double* output_add){
	double min = 9999999;
	int min_loc = -1;
...
int main(){/*
	...
	double input[4] = {1, 1, 0, 0};
	double weight[8] = {0.2,0.6,0.5,0.9,0.8,0.4,0.7,0.3};
	double output[4] = {0};
	double output_add [2] = {0};
	int learning_rate = 0.6;
	// Reset the GPUs
	cudaDeviceReset();
	...
	nn_diff<<< COLUMN_SIZE ,ROW_SIZE >>>(dev_input,dev_weight,dev_output,COLUMN_SIZE); // output = (input - weight)^2
	nn_diff_add<<< 1 ,ROW_SIZE >>>(dev_output,dev_output_add,COLUMN_SIZE); //output_add = Addition of all the columns in a row
    ...
    // Check for min in output_add
	int min_loc = find_minimum(output_add);
	nn_weight_update<<< COLUMN_SIZE ,1 >>>(dev_input,dev_weight,learning_rate, min_loc, COLUMN_SIZE); //output_add = Addition of all the columns in a row
